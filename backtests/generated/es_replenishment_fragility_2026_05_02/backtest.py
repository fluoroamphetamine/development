...
This version keeps the data-loading path compatible with the shared R2 helper
while making the contract's unresolved mechanics explicit in config:
...
- family-specific vs single-family evaluation
"""
        ...
        for obs_minutes in observation_windows:
            ...
            for horizon in continuation_horizons:
                ...
                continuation_return_bp = (
                    10000.0 * impulse_sign * safe_div(exit_close - entry_open, entry_open)
                    if impulse_sign
                    else 0.0
                )
...
def filter_years(df: pl.DataFrame, start_year: int, end_year: int) -> pl.DataFrame:
    return df.filter(
...
def threshold_for_row(
    ...
    key = family_mode
    return thresholds.get(key) or next(iter(thresholds.values()))
...
def evaluate_rule(
    ...
    confusion = confusion_and_accuracy(
        ...
        lambda row: row[f"trust_label_{params['continuation_horizon_minutes']}"] ,
    )
...
def select_best_params(
    ...
    for params in search_space:
        ...
        thresholds = compute_thresholds(
            [row for row in scoped_rows if row_matches_family_mode(row, params["window_family_mode"])],
            params,
        )
        if not thresholds:
            continue
...
def summarize_by_family(evaluation_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (rule, family), rows in sorted(grouped.items()):
        summary = confusion_and_accuracy(
            rows,
            "predicted_trustworthy",
            lambda row: row[f"trust_label_{row['continuation_horizon_minutes']}"] ,
        )
...
def main() -> None:
    ...
    contract_review = {
        ...
        "summary_rows": [
            ...
            {
                "source": "Executable-gate contract",
                "insight": "Confirms compiler-facing gaps around decision time, exit horizon, thresholds, and evaluation protocol.",
                "relevance": "Explains which experiments need to be made explicit.",
            },
            {
                "source": "R2 manifest",
        ...
        "validation_assessment": (
            ...
            "of validation experiments because its main risks are not just parameter tuning risks. "
            "They are structural: whether the proxy adds information beyond volatility, whether the best "
            "observation and holding windows are stable, and whether 08:30 and 09:30 behave like the same family."
        ),
        "added_experiments": [
            {
